...
async def get_recommendations(user_id: int = Depends(jwt_bearer)):
    ...
    recommendations_ids = make_predictions(
        [user_id], "../models/model.pkl", "../data/processed/user_matrix.csv")
    
    # Loading movie data
    movies_data = pd.read_csv("../data/raw/movies.csv")
...
async def get_preferences(user_id: int = Depends(jwt_bearer)):
    ...
    preferences = get_user_preferences(user_id, "../data/processed/user_matrix.csv")
